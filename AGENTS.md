...
7. Patient identity and staff identity are **separate mechanisms** (ADR-0010): patients get an opaque session token from a verified LINE identity (`internal/session`), staff/admin get an argon2id password login returning a JWT access token plus a rotating refresh token (`internal/auth`). Neither token is accepted by the other's surface, and the two must not be merged into one table or one middleware. There is a **third credential kind** (ADR-0011): the visit share link (`internal/share`) — visit-scoped, read-only, time-limited, revocable, no login; valid on `GET /api/v1/shared/journey` only. The cross-token rule holds in every direction: a token works only on the surface its own scheme names, and share tokens are never stored raw (sha256 only) nor placed in URLs. A **fourth, non-identity surface** exists (ADR-0014): the HIS-minted visit link (`internal/visitlink`) — the HIS calls `POST /api/v1/his/visits/{visitId}/patient-link` with `X-HIS-API-Key` to mint a slip QR URL (`#vt=` fragment, never query); the patient redeems it once via `POST /api/v1/journeys/claim` to *grant their session the visit claim* — it is a voucher for the #96 claim, not a credential that reads anything. Links are HMAC-derived (hash-only storage, `rotate=true` revokes, secret rotation revokes all), registered fail-closed (routes exist only when `HIS_API_KEY` and `VISIT_LINK_SECRET` are both set), and the demo-only VN-typed claim `POST /journeys/{visitId}/claim` is gated behind `ALLOW_DEMO_AUTH`.
8. Display text is **client-owned, not server-owned** (ADR-0012): the API sends stable codes (`ServicePoint.code`, step `kind`/`clinicCode`), never localized strings. All patient-facing text lives in typed catalogs under `apps/web/src/i18n/locales/` (`th.ts` canonical, `en.ts` must satisfy the same type — a missing key is a compile error). The DB `name` column stays as the staff-facing label and the fallback for unmapped codes; don't add server-side translation or `Accept-Language` negotiation.
