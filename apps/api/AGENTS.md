...
- The visit link (ADR-0014, `internal/visitlink`) adds two routes, neither a new identity: `POST /api/v1/his/visits/{visitId}/patient-link` is guarded by `X-HIS-API-Key` (`subtle.ConstantTimeCompare` — the key *mints* a credential, it is not one), and `POST /api/v1/journeys/claim` takes `{token}` under `session.RequireSession` and grants the #96 claim for the token's visit. Both register **fail-closed** in `cmd/server/main.go`: unset `HIS_API_KEY`/`VISIT_LINK_SECRET` means the routes do not exist. Link tokens are HMAC-derived from the secret, stored sha256-only, delivered in a `#vt=` URL fragment (never query — the request logger writes `c.Path()`), revoked by `rotate=true` (version bump) or secret rotation, and lifetime-checked lazily against the visit's projected state (COMPLETED stays redeemable for `VISIT_LINK_COMPLETED_GRACE`, default 30m; CANCELLED dies). The VN-typed claim `POST /api/v1/journeys/{visitId}/claim` remains demo-only (`ALLOW_DEMO_AUTH`).
- Fiber v3 `c.Params()` returns zero-copy views into the recycled request buffer — never retain one past the request (the visitlink handler clones `visitId` before it reaches the repo; mock-his has a `param()` helper for the same reason).
