...
เข้าแอปผู้ป่วยผ่าน **LINE LIFF** (กรณีตั้ง `LINE_CHANNEL_ID` + LIFF URL จริง — การเข้าผ่าน LINE ถูกเดินไว้แล้วใน issue #15/#16) หรือ **URL ตรง** สำหรับ demo ในเครื่อง: เปิดแท็บ A (หน้าแรก `/` ส่งไปที่ login เจ้าหน้าที่ — ผู้ป่วยเข้าทาง `/patient/journey` ซึ่งถ้าไม่มี `?visit=` จะขึ้นหน้า **ค้นหาการนัดหมาย** ให้พิมพ์ VN เช่น `VISIT-002` ก่อน — จะสาธิตจริงหรือเปิดลิงก์แท็บ A ตรง ๆ ก็ได้)
