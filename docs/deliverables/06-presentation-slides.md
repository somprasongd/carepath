...
- **Designed, not yet built**: registration + pathway templates (M2/M3), staff queue console (M7), login/RBAC (M8) — schema and screens are designed ([ER Diagram](02-er-diagram-database.md), [Prototype](03-prototype-wireframe.md#33-known-gap--screens-not-yet-designed)), implementation is next
...
| "ข้อมูลผู้ป่วยปลอดภัยแค่ไหน?" (How secure is patient data?) | Passwords would be one-way hashed, queries are parameterized (verified — see [Test Result TC-06](05-test-result.md#test-cases)), and PHI stays off public-facing screens like the queue-call display (NFR-03, NFR-11). | ปฐมพงศ์ |
