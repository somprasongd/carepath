...
### 3.1 Two entry paths — both built; one still needs a real-world rollout step

Frame this as "how does a patient/visitor activate CarePath," not just a QR idea in isolation — both paths converge on the same journey screen. **Correction from an earlier draft of this outline: the QR itself is already built, not a proposal** — verified against `apps/mock-his/internal/mockhis/http.go` (`GET /api/v1/demo/visits/{visitId}/qrcode.png`, generated with `github.com/skip2/go-qrcode`) and `console.html`, which renders that QR per visit in the Mock HIS console detail panel.

- **Path 1 — LINE LIFF (built, in production use).** Open via LINE OA/LIFF directly, or look up by VN (FR-01, FR-02).
- **Path 2 — scan the QR already generated by Mock HIS (built; rollout step remaining is printing it).** The QR encodes an absolute URL straight to that visit's patient journey screen (`qrTarget()` builds `<base>/patient/journey?visit=<visitId>`, `apps/mock-his/internal/mockhis/http.go`) — scanning it or clicking the console's "open →" link opens the same page. Today that QR only exists digitally, inside the Mock HIS console. **What's left is not code**: a real hospital deployment needs that same QR printed onto the hospital's existing physical ใบนำทาง (referral/queue slip handed to the patient at registration) — a rollout/print-integration step, not a development one. This is also a natural fit with the existing QR location-provider pattern (FR-06, [ADR-0004](../adr/0004-location-provider-abstraction.md)); it reuses an entry-point QR rather than adding a new subsystem, and it works precisely because every hospital already has this document — no new artifact to design.
...
- **Location abstraction** — `LocationService` behind one interface with QR / Zigbee / Manual providers; the same pattern the referral-slip QR (section 3.1) reuses as a location/entry provider.

## Open items before this goes to slides

- Section 3.1 (QR on the referral slip): the QR generation and deep link are built (Mock HIS) — the open item is only the print/rollout step with a real hospital, not development. Don't undersell it as "just an idea" if asked "is this built?"
