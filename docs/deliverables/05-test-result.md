...
- **TC-11/TC-13**'s premise ("navigation route endpoint not yet built") no longer holds either: `GET /api/v1/navigation/route` now has a real handler (`apps/api/internal/navigation`), is registered on the router, and `apps/web`'s `/patient/navigate` screen calls it to draw the route on the SVG floor plan with turn-by-turn text cues. Re-run today, TC-11 would return a real route (or a typed 4xx for an unroutable pair) instead of a bare framework 404, and TC-13's "not supported yet" message would only appear for a service point genuinely missing from the navigation graph, not for the feature as a whole.
- **TC-08/TC-09/TC-10's counts are long stale.** The Summary table (22/6/18, 46 total) reflects the suite sizes at the time of the original pass. As of 2026-09-20 the repo has grown to **205** Go tests in `apps/api`, **27** in `apps/mock-his`, and **168** across 16 Vitest files in `apps/web` (~400 automated tests total) — the codebase has added the `analytics`, `auth`, and `share` modules plus their test coverage since this pass ran. This table is not being rewritten to match (see the policy at the top of this section); a fresh test-result pass should be run and recorded as a new deliverable revision if an up-to-date pass/fail count is needed.
