...
identity       (planned)
visit          (implemented)
journey        (planned — next-step logic lives in visit until Pathway exists)
servicepoint   (implemented)
hospitalmap    (planned)
navigation     (planned)
location       (planned)
notification   (planned)
integration    (implemented as internal/his)
```
...
└── internal/
    ...
    ├── his/                # integration module: Client port + httpclient adapter
    │   └── httpclient/
    ├── servicepoint/       # domain type + Repo port + Service
    │   └── postgres/       # pgx adapter implementing the Repo port
    └── visit/              # domain types + Service + HTTP handler
```
